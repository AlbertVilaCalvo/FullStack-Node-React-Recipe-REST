...
if [[ -z "${AWS_ACCOUNT_ID}" ]]; then
  log_error "Could not retrieve AWS account ID. Please check your AWS credentials."
  exit 1
fi
...
# Secrets (DB_PASSWORD, JWT_SECRET, EMAIL_USER, EMAIL_PASSWORD) are NOT injected here;
# they are synced at runtime by the External Secrets Operator from AWS Secrets Manager.
sed -i.bak \
  ...
  -e "s|REPLACE_WITH_RDS_PASSWORD_SECRET_NAME|${APP_NAME}-rds-master-password-${ENVIRONMENT}|g" \
  -e "s|REPLACE_WITH_JWT_SECRET_NAME|${APP_NAME}-jwt-secret-${ENVIRONMENT}|g" \
  -e "s|REPLACE_WITH_EMAIL_USER_SECRET_NAME|${APP_NAME}-email-user-${ENVIRONMENT}|g" \
  -e "s|REPLACE_WITH_EMAIL_PASSWORD_SECRET_NAME|${APP_NAME}-email-password-${ENVIRONMENT}|g" \
