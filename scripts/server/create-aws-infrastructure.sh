...
# Step 3: Pre-download Helm charts and install Kubernetes controllers
# Terraform's Helm provider uses Go's HTTP client which times out when fetching
# charts from GitHub CDN (185.199.x.x). Pre-downloading sequentially with the
# helm CLI (which is unaffected) eliminates all remote fetches during terraform apply.
log_step "Step 3/5: Pre-downloading Helm charts and installing Kubernetes controllers..."
