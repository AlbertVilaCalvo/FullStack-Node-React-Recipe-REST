...
data "aws_route53_zone" "root_zone" {
  ...
  # Assuming all endpoints belong to the same root hosted zone
...
resource "helm_release" "external_dns" {
  ...
  values = [
    yamlencode({
      ...
      domainFilters = [data.aws_route53_zone.root_zone.name]
      ...
      extraArgs = [
        ...
        "--zone-id-filter=${data.aws_route53_zone.root_zone.zone_id}",
