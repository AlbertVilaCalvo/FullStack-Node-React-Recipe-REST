...
#    terraform apply -target=module.vpc -target=module.eks -target=module.rds -target=module.ecr -target=module.pod_identity -target=module.api_endpoint_certificate -target=module.argocd_endpoint_certificate -target=module.app_secrets
...
module "argocd_endpoint_certificate" {
  source = "../../modules/api-endpoint-certificate"

  app_name    = var.app_name
  environment = var.environment

  endpoint      = var.argocd_endpoint
  endpoint_name = "argocd"
}
...
module "external_dns" {
  ...
  cluster_name      = module.eks.cluster_name
  managed_endpoints = [var.api_endpoint, var.argocd_endpoint]
}
