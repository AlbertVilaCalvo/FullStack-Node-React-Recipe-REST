...
variable "additional_default_tags" {
  description = "Additional default tags, in addition to Application"
  type        = map(string)
  default     = {}
  validation {
    condition     = length([for k in keys(var.additional_default_tags) : k if k == "Application"]) == 0
    error_message = "additional_default_tags must not contain the key \"Application\" as it is already set."
  }
}
